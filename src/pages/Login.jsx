...
const Login = () => {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      const res = await phoneAuth({ token, action: 'login' });
        ...
        <div className="relative z-10 space-y-12 max-w-xl">
          <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }}>
             ...
             <h1 className="text-7xl font-black text-white leading-none tracking-tighter">
               <span className="text-emerald-500 italic">Resume</span> Your Vote.
             </h1>
          </motion.div>
          <p className="text-xl text-slate-500 font-medium leading-relaxed">
            Verify your secure connection to re-enter Kerala's prediction arena.
          </p>
        </div>
      </div>
        ...
        >
          <div className="space-y-8">
            <h2 className="text-4xl font-black tracking-tight italic">
              {step === 1 ? 'Node Login' : 'Verify Connection'}
            </h2>
                ...
                >
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                      <Smartphone size={14} /> Registered Phone Number
                    </label>
        ...
        {step === 1 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-8 text-center relative z-10 border-t border-white/5 pt-8 w-full max-w-md">
            <p className="text-slate-500 text-sm font-medium">
              New archivist?{' '}
              <Link to="/signup" className="text-emerald-500 font-bold hover:underline transition-all">Secure an Identity</Link>
            </p>
          </motion.div>
        )}
      </div>
